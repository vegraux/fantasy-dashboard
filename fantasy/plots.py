...
def get_variable_by_player_per_manager(
    ...
    zeros = [player for player, b in (points_per_player.groupby("web_name")[variable].sum() == 0).items() if b]
    points_per_player = points_per_player[~points_per_player["web_name"].isin(zeros)]
